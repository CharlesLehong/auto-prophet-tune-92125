  ...
  const generateHTML = async () => {
    try {
      ...
      for (const segment of results.segments) {
        ...
        charts[segment.segment] = images.join('');
      }
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => handleExportClick('csv')}>
                <Download className="h-4 w-4 mr-2" />
                CSV
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleExportClick('html')}>
                <Download className="h-4 w-4 mr-2" />
                HTML
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleExportClick('pdf')}>
                <Download className="h-4 w-4 mr-2" />
                PDF
              </Button>
            </div>
      ...
      <Dialog open={exportDialogOpen} onOpenChange={setExportDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Name your {exportType.toUpperCase()} report before exporting
            </DialogDescription>
          </DialogHeader>
      ...
      <Tabs defaultValue={results.segments[0]?.segment || "0"} className="space-y-4">
        ...
        {results.segments.map((segment, idx) => (
          <TabsContent key={idx} value={segment.segment} className="space-y-6" data-segment={segment.segment}>
